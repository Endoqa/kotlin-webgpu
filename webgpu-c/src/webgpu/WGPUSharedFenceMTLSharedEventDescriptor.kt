...
public value class WGPUSharedFenceMTLSharedEventDescriptor(
    ...
    public var chain: WGPUChainedStruct
        get() =
            WGPUChainedStruct(
                WGPUSharedFenceMTLSharedEventDescriptor.chainHandle.invokeExact(
                    this.`$mem`,
                    0L
                ) as MemorySegment
            )
    ...
    public var sharedEvent: Pointer<Unit>
        get() = WGPUSharedFenceMTLSharedEventDescriptor.sharedEventHandle.get(this.`$mem`, 0L) as
                MemorySegment
        set(`value`) {
            WGPUSharedFenceMTLSharedEventDescriptor.sharedEventHandle.set(this.`$mem`, 0L, value)
        }

    public constructor(gc: Boolean) : this(kotlin.run {
        require(gc) { "Do not call this if gc is not want" }
        Arena.ofAuto().allocate(WGPUSharedFenceMTLSharedEventDescriptor.layout)
    })
    ...
    public companion object {
        ...
        public fun allocate(alloc: SegmentAllocator): WGPUSharedFenceMTLSharedEventDescriptor =
            WGPUSharedFenceMTLSharedEventDescriptor(alloc.allocate(WGPUSharedFenceMTLSharedEventDescriptor.layout))
    }
}
