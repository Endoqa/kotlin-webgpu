...
public value class WGPUBufferHostMappedPointer(
    ...
    public var pointer: Pointer<Unit>
        get() = pointerHandle.get(this.`$mem`, 0L) as MemorySegment
        set(`value`) {
            pointerHandle.set(this.`$mem`, 0L, value)
        }

    public var disposeCallback: WGPUCallback
        get() = disposeCallbackHandle.get(this.`$mem`, 0L) as MemorySegment
        set(`value`) {
            disposeCallbackHandle.set(this.`$mem`, 0L, value)
        }
    ...
    public companion object {
        ...
        public fun allocate(alloc: SegmentAllocator): WGPUBufferHostMappedPointer =
            WGPUBufferHostMappedPointer(alloc.allocate(layout))
    }
}
