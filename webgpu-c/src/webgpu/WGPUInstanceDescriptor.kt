...
public value class WGPUInstanceDescriptor(
    ...
    public var features: WGPUInstanceFeatures
        get() = WGPUInstanceFeatures(
            WGPUInstanceDescriptor.featuresHandle.invokeExact(this.`$mem`, 0L)
                    as MemorySegment
        )
        set(`value`) {
            MemorySegment.copy(
                value.`$mem`, 0L, this.features.`$mem`, 0L,
                WGPUInstanceFeatures.layout.byteSize()
            )
        }
