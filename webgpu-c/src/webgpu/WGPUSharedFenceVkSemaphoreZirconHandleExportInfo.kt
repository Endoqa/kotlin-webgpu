...
public value class WGPUSharedFenceVkSemaphoreZirconHandleExportInfo(
    ...
    public var chain: WGPUChainedStructOut
        get() =
            WGPUChainedStructOut(
                WGPUSharedFenceVkSemaphoreZirconHandleExportInfo.chainHandle.invokeExact(
                    this.`$mem`,
                    0L
                ) as MemorySegment
            )
    ...
    public var handle: uint32_t
        get() = (WGPUSharedFenceVkSemaphoreZirconHandleExportInfo.handleHandle.get(this.`$mem`, 0L) as
                Int).toUInt()
        set(`value`) {
            WGPUSharedFenceVkSemaphoreZirconHandleExportInfo.handleHandle.set(
                this.`$mem`,
                0L, value.toInt()
            )
        }
    ...
    public constructor(gc: Boolean) : this(kotlin.run {
        ...
        Arena.ofAuto().allocate(WGPUSharedFenceVkSemaphoreZirconHandleExportInfo.layout)
    })
    ...
    public companion object {
        ...
        public fun allocate(alloc: SegmentAllocator): WGPUSharedFenceVkSemaphoreZirconHandleExportInfo =
            WGPUSharedFenceVkSemaphoreZirconHandleExportInfo(
                alloc.allocate(
                    WGPUSharedFenceVkSemaphoreZirconHandleExportInfo.layout
                )
            )
    }
}
