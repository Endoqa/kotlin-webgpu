...
public value class WGPUSurfaceSourceMetalLayer(
    ...
    public var chain: WGPUChainedStruct
        get() = WGPUChainedStruct(
            WGPUSurfaceSourceMetalLayer.chainHandle.invokeExact(this.`$mem`, 0L)
                    as MemorySegment
        )
        set(`value`) {
            MemorySegment.copy(
    ...
    public var layer: Pointer<Unit>
        get() = WGPUSurfaceSourceMetalLayer.layerHandle.get(this.`$mem`, 0L) as MemorySegment
        set(`value`) {
            WGPUSurfaceSourceMetalLayer.layerHandle.set(this.`$mem`, 0L, value)
        }

    public constructor(gc: Boolean) : this(kotlin.run {
        require(gc) { "Do not call this if gc is not want" }
        Arena.ofAuto().allocate(WGPUSurfaceSourceMetalLayer.layout)
    })
    ...
    public companion object {
        ...
        public fun allocate(alloc: SegmentAllocator): WGPUSurfaceSourceMetalLayer =
            WGPUSurfaceSourceMetalLayer(alloc.allocate(WGPUSurfaceSourceMetalLayer.layout))
    }
}
