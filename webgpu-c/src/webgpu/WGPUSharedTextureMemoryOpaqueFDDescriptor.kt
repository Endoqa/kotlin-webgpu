...
public value class WGPUSharedTextureMemoryOpaqueFDDescriptor(
    ...
    public var vkImageCreateInfo: Pointer<Unit>
        get() = vkImageCreateInfoHandle.get(this.`$mem`, 0L)
                as MemorySegment
        set(`value`) {
            vkImageCreateInfoHandle.set(this.`$mem`, 0L, value)
        }

    public var memoryFD: Int
        get() = memoryFDHandle.get(this.`$mem`, 0L) as Int
        set(`value`) {
            memoryFDHandle.set(this.`$mem`, 0L, value)
        }

    public var memoryTypeIndex: uint32_t
        get() = (memoryTypeIndexHandle.get(this.`$mem`, 0L) as
                Int).toUInt()
        set(`value`) {
            memoryTypeIndexHandle.set(
                this.`$mem`,
                0L, value.toInt()
            )
        }
    ...
    public var allocationSize: uint64_t
        get() = (allocationSizeHandle.get(this.`$mem`, 0L) as
                Long).toULong()
        set(`value`) {
            allocationSizeHandle.set(
                this.`$mem`,
                0L, value.toLong()
            )
        }
    ...
    public var dedicatedAllocation: WGPUBool
        get() = (dedicatedAllocationHandle.get(
            this.`$mem`,
            0L
        ) as Int).toUInt()
        set(`value`) {
            dedicatedAllocationHandle.set(
                this.`$mem`,
                0L, value.toInt()
            )
        }
    ...
    public companion object {
        ...
        public fun allocate(alloc: SegmentAllocator): WGPUSharedTextureMemoryOpaqueFDDescriptor =
            WGPUSharedTextureMemoryOpaqueFDDescriptor(alloc.allocate(layout))
    }
}
